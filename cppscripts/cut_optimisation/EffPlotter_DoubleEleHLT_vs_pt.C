...
void EffPlotter_DoubleEleHLT_vs_pt() {

  TFile *file_sig = new TFile("hist_Zprime_DoubleEle_L1seeded.root");
  ...
  TH1D* num_sig_leg1 = (TH1D*)file_sig->Get("num_ele_genpt_EB_leg1");
  TH1D* num_sig_leg2 = (TH1D*)file_sig->Get("num_ele_genpt_EB_leg2");
  ...
  TH1D* num_sig_leg1_EE = (TH1D*)file_sig->Get("num_ele_genpt_EE_leg1");
  TH1D* num_sig_leg2_EE = (TH1D*)file_sig->Get("num_ele_genpt_EE_leg2");
  ...
  if(TEfficiency::CheckConsistency(*num_sig_leg1_EE,*den_sig_EE)) {
    ...
    pEff_sig_leg1_EE->SetLineWidth(3);
  ...
  if(TEfficiency::CheckConsistency(*num_sig_leg2_EE,*den_sig_EE)) {
    ...
    pEff_sig_leg2_EE->SetLineWidth(2);
    //    pEff_sig_leg2->SetTitle("Signal efficiency vs gen pT;gen pT [GeV];efficiency");
    pEff_sig_leg2_EE->Draw("same");
    gPad->Update();
  }

  TLegend *leg_example22 = new TLegend(0.4,0.25,0.9,0.55);
  leg_example22->SetHeader("L1+HLT efficiency","C"); // option "C" allows to center the header
  leg_example22->AddEntry((TObject*)0, "HLT_DoubleEle25_CaloIdL_PMS2", "");
  ...
  my_canvas1->SaveAs("DoubleEle_fullEff_vs_genpt_EB_leg_1_2_zprime.png");
  my_canvas2->SaveAs("DoubleEle_fullEff_vs_genpt_EE_leg_1_2_zprime.png");
