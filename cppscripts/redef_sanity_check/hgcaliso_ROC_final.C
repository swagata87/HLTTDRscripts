...
int hgcaliso_ROC_final() {

  TFile *file_bkg = new TFile("redef_bkg_Dec.root");
  TFile *file_sig = new TFile("redef_sig_Dec.root");
  /////

  TH1D* h1_sig_old = (TH1D*)file_sig->Get("hgcaliso_old_sig_EE");
  TH1D* h1_bkg_old = (TH1D*)file_bkg->Get("hgcaliso_old_bkg_EE");
  ...
  */
  ...
  TLegend *leg_example = new TLegend(0.17,0.17,0.65,0.5);
  ...
  c1->SetGrid();
  c1->SaveAs("ROC_HGCAL_isolation.png");
