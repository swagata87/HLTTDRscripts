...
int ecaliso_ROC() {
  ...
  TH1D* h1_sig_old = (TH1D*)file_sig->Get("ecal_iso_old_sig");
  TH1D* h1_bkg_old = (TH1D*)file_bkg->Get("ecal_iso_old_bkg");

  TH1D* h1_sig_new = (TH1D*)file_sig->Get("ecal_iso_new_sig");
  TH1D* h1_bkg_new = (TH1D*)file_bkg->Get("ecal_iso_new_bkg");
  ...
  */
  ...
  TPaveText *t = new TPaveText(0.95,0.65,1.00,0.72);
  ...
  c1->SetGrid();
  c1->SaveAs("ROC_ecaliso_new_vs_old_Barrel.png");
