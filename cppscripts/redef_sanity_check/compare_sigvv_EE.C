...
void compare_sigvv_EE() {
  ...
  TH1D* h1_sig = (TH1D*)file_sig->Get("sigmavv_sig_EE");
  h1_sig->Rebin(2);
  ...
  h1_sig->GetXaxis()->SetTitle("#sigma_{vv}");
  h1_sig->GetYaxis()->SetTitle("Arbitrary unit");
  h1_sig->SetTitle("");
  h1_sig->SetStats(0);
  h1_sig->GetXaxis()->SetRangeUser(0.4, 2.0);

  TH1D* h1_bkg = (TH1D*)file_bkg->Get("sigmavv_bkg_EE");
  h1_bkg->Rebin(2);
  ...
  //  h1_bkg->GetXaxis()->SetTitle("#sigmavv");
  //h1_bkg->SetTitle("");
  //h1_bkg->SetStats(0);
  h1_bkg->GetXaxis()->SetRangeUser(0.4, 2.0);


  TCanvas* my_canvas1 = new TCanvas("canvas1","canvas1",50, 50, 800,600);
  my_canvas1->cd();
  gPad->SetLogy();
  h1_sig->Draw("hist");
  ...
  leg_example->SetHeader("Electron #sigma_{vv}, HGCAL","C"); // option "C" allows to center the header
  leg_example->AddEntry((TObject*)0, "p_{T}>30 GeV, PU=200", "");
  ...
  TPaveText *t = new TPaveText(0.34,0.26,1.8,0.4);
  ...
  my_canvas1->SaveAs("sigmavv_EE_sig_bkg.png");
